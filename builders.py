...
def build_ytd_summary_compact(ws, income_merged, expense_merged, org_name,
    ...
    cell stays a real, summable number (a single cell can't hold two
    numbers and still work in an Excel SUM() the way a treasurer
    select-and-totaling a column would expect). Section TOTAL rows are
    the one exception -- those are synthetic aggregates already, so their
    ...
    # Both figures reflect PTA money -- net of any READTHON (or other
    # pass-through fund) balance held in the account -- rather than the
    # raw bank balance, so this row can't be misread as "how much cash
    # is in the account" when part of that cash isn't really the PTA's.
    # Balance Forward: caller is expected to have already subtracted the
    # pass-through balance held AS OF the fiscal year's own July 1 (not
    # today's cumulative figure) before passing it in here. Current
    # Balance: computed here directly, since this function already has
    # both bank and readthon in hand.
    readthon_balance_held = readthon['balance_held'] if readthon else 0.0
    current_balance = bank['ending_balance'] - readthon_balance_held
